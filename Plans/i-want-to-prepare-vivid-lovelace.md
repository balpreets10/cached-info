...
- **Phase 6 — Frontend data cutover + Supabase removal: ⏳ PENDING (next).** Pages/components still read the Supabase `DataContext` (`HomePage`, `Resources`, `SubmitResource`, `RequestResource`) and admin tabs still call `supabase` directly. Port these to the React Query hooks + service modules, then delete `DataContext.jsx` + `supabaseClient.js`, drop `@supabase/supabase-js` + `@react-oauth/google` + `jwt-decode`, and remove the `DataProvider` temporary wrapper in `app/App.jsx`. Then **Phase 7** (deploy).
  - Note: Google OAuth creds not yet configured locally, so end-to-end login can't be smoke-tested until they're added to `server/.env.development` (see the setup guide).
